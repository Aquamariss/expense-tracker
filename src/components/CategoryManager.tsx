...
export default function CategoryManager({ categories, expenses, onAdd, onRename, onDelete, onClose }: Props) {
  ...
  function expenseCount(cat: string) {
  ...
  function commitEdit() {
    ...
    if (trimmed && trimmed !== oldName) {
      if (categories.includes(trimmed)) {
        setError(`Категория «${trimmed}» уже существует`);
        return;
      }
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="px-6 pt-5 pb-4 flex items-center justify-between border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            ...
            <svg viewBox="0 0 24 24" fill="none" stroke="#6366f1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
              ...
              <polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76" fill="#6366f1" fillOpacity="0.15" />
            </svg>
            <h2 className="text-base font-semibold text-gray-900">Управление категориями</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-3 max-h-80 overflow-y-auto space-y-1">
          {categories.map((cat, i) => {
            ...
            return (
              <div key={cat} className="flex items-center gap-2 group rounded-xl px-1 py-0.5 hover:bg-gray-50 transition-colors">
                {editingIndex === i ? (
                  <input
                    ref={editRef}
                    value={editValue}
                ...
                {/* Delete button */}
                <button
                  onClick={() => count === 0 && onDelete(cat)}
                  disabled={count > 0}
                  title={count > 0 ? `${count} трат — нельзя удалить` : "Удалить категорию"}
        ...
        {/* Error */}
        {error && (
          <p className="px-6 pb-1 text-xs text-red-500">{error}</p>
        )}

        {/* Add new */}
        <div className="px-6 py-4 border-t border-gray-100 space-y-2">
          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Новая категория</p>
